...
import { newsI18n, regionLabelI18n } from "@/data/bda/i18n";
          ...
          <section key={region} className="border-t bda-hairline">
            <div className="mx-auto max-w-6xl px-5 py-14 sm:px-8 sm:py-20">
              ...
              <div className="mt-9 grid gap-5 sm:grid-cols-2">
                {items.map((item) => {
                  const tr = newsI18n[item.id];
                  const headline = locale === "es" ? item.headline : tr?.headline[locale] ?? item.headline;
                  const summary = locale === "es" ? item.summary : tr?.summary[locale] ?? item.summary;
                  return (
                    <a
                      key={item.id}
                      href={item.url}
